...
employees   : 14行 (hire_date カラム名に注意 — 本演習の schema では hired_at と表記)
...
> **注意**: このページのスキーマ例では `hired_at` と記載していますが、実際のサンプルデータでは `hire_date` カラムを使用しています。クエリ実行時は `hire_date` に読み替えてください。
